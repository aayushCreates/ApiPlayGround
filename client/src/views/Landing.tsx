...
const features = [
  {
    ...
    description:
      "Import OpenAPI specs easily and automatically generate interactive forms for all your endpoints.",
    icon: Globe,
    color: "text-accent",
  ...
  },
  {
    title: "Blazing Fast",
    description:
      "Built with Vite and React for snappy response times and instant feedback on all your API calls.",
    icon: Zap,
    color: "text-success",
  ...
  },
  {
    title: "Secure & Sync",
    description:
      "Your data safely synced and authenticated through state of the art protection. Access anywhere.",
    icon: Lock,
    color: "text-warning",
  ...
  return (
    <div className="min-h-screen bg-base text-primary overflow-x-hidden relative font-sans flex flex-col">
        ...
        <div className="fixed inset-0 z-40 bg-base/95 backdrop-blur-lg md:hidden flex flex-col items-center justify-center gap-8 p-6 animate-in fade-in zoom-in duration-200">
          <div className="flex flex-col items-center gap-6 w-full max-w-xs">
            {!isLoaded ? null : isSignedIn ? (
              ...
              <>
                <SignInButton mode="modal">
                  <button className="text-xl font-semibold text-secondary hover:text-primary transition-colors w-full py-4 border-b border-border">
                    Sign In
                  </button>
                </SignInButton>
                <SignUpButton mode="modal">
                  <button className="bg-accent text-white w-full py-2 rounded-lg text-xl font-semibold shadow-lg shadow-accent/20">
                    Get Started Free
                  </button>
                </SignUpButton>
              </>
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-6 pt-16 md:pt-24 pb-32 flex flex-col items-center text-center flex-1">
        ...
        <h1 className="text-3xl sm:text-4xl md:text-6xl font-extrabold tracking-tight mb-6 max-w-5xl text-pretty leading-[1.1]">
          Test, Share, and Discover APIs{" "}
          <span className="text-transparent bg-clip-text bg-linear-to-r from-accent to-[#b4c6ff]">
            with ease.
          </span>
        </h1>

        <p className="text-lg md:text-xl text-secondary mb-12 max-w-2xl text-pretty leading-relaxed px-4">
          A blazingly fast, modern API playground designed for developers.
          Import specifications, run requests, and collaborate seamlessly.
        </p>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto px-6">
          {!isLoaded ? (
            ...
            <Link
              to="/app"
              className="w-full sm:w-auto bg-accent hover:bg-accent-hover text-white px-8 py-3.5 rounded-lg font-medium transition-all flex items-center justify-center gap-2 group text-lg shadow-lg shadow-accent/20 cursor-pointer"
            >
              Go to App
          ...
          <a
            href="#features"
            className="w-full sm:w-auto px-8 py-3.5 rounded-lg font-medium text-primary hover:bg-elevated transition-colors border border-border bg-surface shadow-sm cursor-pointer border-b-2"
          >
            Explore Features
          </a>
        </div>
          ...
          <div className="rounded-xl overflow-hidden border border-border-subtle bg-base aspect-[16/10] md:aspect-video flex flex-col relative shadow-inner">
            ...
            <div className="w-full h-10 md:h-12 border-b border-border bg-surface/90 flex items-center px-4 gap-3 z-10 backdrop-blur-md">
              ...
              <div className="mx-auto w-32 sm:w-48 md:w-64 h-5 md:h-6 bg-base rounded-md border border-border flex items-center justify-center overflow-hidden">
                ...
                <span className="text-[10px] md:text-xs text-secondary mono truncate">
                  apiplayground.app
                </span>
              </div>
            </div>
            ...
            <div className="flex-1 flex text-secondary relative z-0 overflow-hidden">
              ...
              <div className="flex-1 p-3 md:p-6 flex flex-col gap-3 md:gap-4 bg-base overflow-x-auto">
                <div className="flex flex-col sm:flex-row gap-2 mb-1">
                  <div className="h-9 md:h-11 w-full sm:w-20 md:w-24 bg-method-post/20 border border-method-post/30 rounded-lg text-method-post flex items-center justify-center font-bold text-xs md:text-sm shadow-sm">
                    POST
                  </div>
                  <div className="h-9 md:h-11 flex-1 bg-surface border border-border rounded-lg px-3 md:px-4 flex items-center shadow-inner overflow-hidden">
                    <span className="text-secondary mono text-[10px] md:text-sm opacity-50 truncate">
                      https://api.example.com/v1/users
                    </span>
                  </div>
                  <div className="h-9 md:h-11 w-full sm:w-24 md:w-28 bg-accent border border-accent rounded-lg text-white flex items-center justify-center font-bold text-xs md:text-sm shadow-md">
                    Send
                  </div>
                </div>
                ...
                <div className="flex-1 border border-border rounded-xl bg-surface/50 p-3 md:p-4 shadow-inner flex flex-col gap-2 relative overflow-hidden min-h-[150px]">
                  <div className="absolute top-0 right-0 left-0 h-8 md:h-10 border-b border-border bg-elevated flex items-center px-4 gap-4 overflow-x-auto whitespace-nowrap scrollbar-hide">
                    <span className="text-[10px] md:text-xs font-semibold text-primary border-b-2 border-accent pb-2 mt-2">
                      Body
                    </span>
                    <span className="text-[10px] md:text-xs font-medium text-secondary pb-2 mt-2">
                      Headers (2)
                    </span>
                    <span className="text-[10px] md:text-xs font-medium text-secondary pb-2 mt-2">
                      Auth
                  ...
                  <div className="pt-10 md:pt-12 font-mono text-[10px] md:text-xs text-primary flex gap-3 md:gap-4">
                    ...
                    <div className="truncate">
                      ...
                      : <span className="text-[#ce9178]">"Dev"</span>,<br />
                      <span className="text-secondary">&#125;</span>
                    </div>
                  </div>
                </div>
        ...
        <div
          id="features"
          className="pt-24 md:pt-32 pb-16 w-full grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 text-left"
        >
          {features.map((feature, index) => (
            <div
              key={index}
              className="p-8 border border-border rounded-2xl bg-surface/40 hover:bg-surface/60 transition-all shadow-sm hover:shadow-xl hover:shadow-accent/5 group relative overflow-hidden"
            >
              <div
                className={`w-14 h-14 rounded-xl ${feature.bgColor} flex items-center justify-center ${feature.color} mb-6 border ${feature.borderColor} group-hover:scale-110 transition-transform duration-300`}
              >
              ...
              <div
                className={`absolute -bottom-10 -right-10 w-32 h-32 ${feature.bgColor} blur-[50px] rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500`}
              />
            </div>
          ))}
        </div>
